...
# Calculate the new variable using normalized values and include the distance adjustment
max_distance = data["Distance to Niagara Falls (miles)"].max()
data["New_Variable"] = sum(
    normalized_data[col] * coefficients[col] * (1 - p_values[col])
    for col in selected_columns_mapped
) * (data["Distance to Niagara Falls (miles)"] / max_distance)
...
plt.title("Calculated Variable by State (Adjusted by Distance to Niagara Falls)", fontsize=16)
plt.axis("off")
...
cbar.set_label("Calculated Variable (Adjusted by Distance)", fontsize=12)
